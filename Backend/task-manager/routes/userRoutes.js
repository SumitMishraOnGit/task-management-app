...
router.post("/login", async (req, res, next) => {
  try {
    ...
    res.status(200).json({ message: "Login successful", accessToken, refreshToken, user: { name: user.name, email: user.email, avatar: user.file } });
  } catch (error) {
    next(error);
  }
});
...
router.put("/profile", verifyToken, uploadProfilePic.single("avatar"), async (req, res, next) => {
  try {
    ...
    if (req.file) user.file = `uploads/profile/${req.file.filename}`;
