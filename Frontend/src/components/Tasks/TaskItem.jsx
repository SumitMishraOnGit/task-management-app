...
const TaskItem = ({ task, toggleTaskStatus, openTaskDetails, deleteTask, isHighlighted, activityType }) => {
  ...
  return (
    <>
      {/* ✨ FIX: Removed the `initial` and `animate` props to disable the entrance animation */}
      <motion.div
        layout
      ...
      <DeleteConfirmationModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={() => deleteTask(task._id)} taskTitle={task.title} />
    </>
  );
};
